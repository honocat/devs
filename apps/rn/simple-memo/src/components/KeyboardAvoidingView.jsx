...
export default function KeyboardSafeView({ children, style }) {
  ...
  const [viewHeignt, setViewHeight] = useState(null);
  ...
  useEffect(() => {
    if (
      [initialViewHeight, animatedViewHeight, viewHeignt].some(
        (val) => val === null,
      )
    ) {
      return;
    ...
    if (viewHeignt === initialViewHeight.current) {
  ...
  function handleShow({ endCoordinates }) {
